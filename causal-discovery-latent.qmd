...
# Compact node labels for all DAG/MAG/PAG plots in this chapter.
plot_attrs <- list(
  node  = list(fontsize = 10, shape = "plaintext", height = 0.3, width = 0.3),
  edge  = list(arrowsize = 0.6),
  graph = list(rankdir = "TB")
)

# Wrap an arbitrary PAG adjacency matrix (pcalg convention) in a minimal
# fciAlgo S4 object so pcalg's plot() draws proper arrow/circle/bidirected
# marks. Used for the "true MAG" panel where we have no algorithm fit.
amat_to_fciAlgo <- function(amat, n_obs = 0L) {
  p <- nrow(amat)
  new("fciAlgo",
      amat             = amat,
      allPdsep         = vector("list", p),
      n.edgetestsPDSEP = numeric(0),
      max.ordPDSEP     = -1L,
      call             = quote(amat_to_fciAlgo()),
      n                = as.integer(n_obs),
      max.ord          = -1L,
      n.edgetests      = numeric(0),
      sepset           = vector("list", p),
      pMax             = matrix(NA_real_, p, p))
}
```
...
# Box-shaped nodes for the latent variables on the full DAG.
attrs_full <- modifyList(plot_attrs,
                         list(node = list(shape = "plaintext", fontsize = 10)))
node_attrs <- list(shape    = setNames(rep("box", length(latent_idx)),
                                       all_labels[latent_idx]),
                   fontsize = setNames(rep(10,   length(latent_idx)),
                                       all_labels[latent_idx]))

op <- par(mfrow = c(1, 2), mar = c(1, 1, 3, 1))
plot(true_dag, attrs = attrs_full, nodeAttrs = node_attrs,
     main = "Full DAG (boxed nodes hidden)")
plot(amat_to_fciAlgo(amat_true_pag, n_obs), main = "True MAG (observed)",
     attrs = plot_attrs)
par(op)
```
...
#| fig-width: 5.5
#| fig-height: 4.5
plot(fci_fit, main = "", attrs = plot_attrs)
```
...
plot(amat_to_fciAlgo(amat_true_pag, n_obs),
     main = "True MAG",          attrs = plot_attrs)
plot(fci_fit,  main = "FCI — estimated PAG",  attrs = plot_attrs)
plot(rfci_fit, main = "RFCI — estimated PAG", attrs = plot_attrs)
par(op)
```
