...
# Smaller node-label font for all DAG/MAG/PAG plots in this chapter.
plot_attrs <- list(node = list(fontsize = 10))
...
# Latent nodes shown with a box shape to flag them as unobserved.
node_attrs <- list(shape = setNames(rep("box", length(latent_idx)),
                                    all_labels[latent_idx]))

op <- par(mfrow = c(1, 2), mar = c(1, 1, 3, 1))
plot(true_dag, attrs = plot_attrs, nodeAttrs = node_attrs,
