...
We reuse the same 8-node Gaussian linear DAG from the previous chapter --- same
seed, same `randomDAG` call with edge probability 0.35 and coefficients drawn
from $[0.25, 1]$, same $n = 2000$ draws with standard normal errors --- and then
hide two of the nodes, $X_3$ and $X_6$, treating them as unobserved
confounders. The algorithms below see a $2000 \times 6$ matrix; the full
8-column matrix is kept only to construct the truth.

The aim is to see what latent confounding costs. The previous chapter's PC
reached a skeleton F1 of 0.952 with all eight variables observed. Nothing about
the DGP changes here except which columns we are allowed to look at.
...
This is the first thing to notice, before any algorithm runs. The full DAG has
11 edges among 8 nodes. The true MAG over the 6 observed nodes has 10 edges
among 15 possible pairs, a density of two thirds. Hiding two variables did not
simplify the problem; it made the graph much denser, because every pair of
observed nodes sharing a hidden parent acquires an edge that was not there
before. That densification is the cost of latent confounding, and it is why the
F1 scores below sit well under the 0.952 that PC managed with everything
observed.
...
FCI recovers the skeleton with an F1 of 0.824, using 165 CI tests. That is a
long way below PC's 0.952 on the same underlying DAG with nothing hidden, and
the dense MAG is why: with two thirds of all pairs adjacent, there are few
conditional independences left to find, and each missed edge costs more.

```{r}
#| label: fig-fci-pag
#| fig-cap: "PAG estimated by FCI. Dashed edges indicate endpoints with circle (○) marks — orientations the algorithm could not determine. Solid double-headed arrows (↔) mark edges with an invariant arrowhead at both ends, which under the maintained assumptions indicates a latent confounding path rather than one identified hidden variable."
#| fig-width: 5.5
#| fig-height: 4.5
...
- A **double-headed arrow** $X \leftrightarrow Y$ has an invariant arrowhead at both ends. *Under the same maintained model*, neither variable is an ancestor of the other in any MAG in the class, and the association between them runs through a latent inducing path. Read it as confounding that no observed adjustment set removes. It does not tell you that one particular hidden variable exists, how many there are, or what they are.
- A **circle endpoint** is the $\circ$ mark — uncertain at that endpoint.

In practice for economic research: focus first on $\leftrightarrow$ edges. Under the maintained assumptions they mark confounding that adjustment on the observed variables cannot remove, which is where a design-based remedy is needed — an instrument, a proxy, a front-door route, or a panel design. Then examine $\to$ edges, the causal claims that survive across all statistically equivalent structures.
...
RFCI reaches the same skeleton F1 of 0.824 using 138 CI tests instead of 165, a
saving of 16%. On this dataset the Possible-D-SEP stage bought FCI nothing:
it ran 27 extra tests and removed no edge that mattered for the skeleton score.
...
The two algorithms tie on skeleton recovery and RFCI is cheaper. One dataset is
not enough to conclude that, which is what the Monte Carlo below is for.
...
We repeat the whole exercise 100 times, drawing a fresh random DAG each time
and choosing a fresh pair of nodes to hide, so the results do not depend on one
graph or one unlucky choice of which variables are unobserved.
...
Averaged over 100 replications, FCI gets a skeleton F1 of 0.938 and RFCI 0.943,
while FCI uses 150.5 CI tests and RFCI 102.7. RFCI is 32% cheaper and loses
nothing on the skeleton.

Two cautions on reading that. The 0.005 F1 difference is not evidence that RFCI
recovers skeletons better; RFCI converges to a *supergraph* of the PAG
skeleton, so if anything theory points the other way, and a gap this small on
100 replications is noise. And the single dataset above scored 0.824 for both,
well below these averages --- that draw was one of the harder ones, as its
denser-than-average MAG would suggest.

What does survive is the cost result. Skipping the Possible-D-SEP stage is
where RFCI's savings come from, and on these graphs that stage is roughly a
third of FCI's work while contributing nothing measurable to skeleton accuracy.
...
The two histograms overlap, which is the point of the figure's caption: RFCI is
usually but not always cheaper, because its extra collider-verification tests
can outweigh the Possible-D-SEP saving on sparse draws.
...
FCI's PAG output distinguishes invariant tails and arrowheads: ancestral relations that hold in every MAG in the class ($\to$), edges bidirected in every MAG and so attributable to a latent path ($\leftrightarrow$), and endpoints the data cannot orient ($\circ$). All three readings are conditional on the maintained discovery assumptions. The PAG can be combined with background knowledge such as temporal ordering to further orient edges before estimation.
