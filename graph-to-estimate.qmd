...
Step 4 is where this workflow most often goes wrong, so it is worth stating
plainly: a CPDAG is an equivalence class, not a graph. An empty adjustment-set
result for a CPDAG is not permission to pass one arbitrary orientation to the ID
algorithm. The [causal discovery chapter](causal-discovery.qmd) sets out what to
do instead.
