...
The front-door formula recovers the effect even though $U$ is unobserved.
The naive mean difference is biased by the path through $U$.
...
`causaleffect` raises an error ("Not identifiable"), which the `tryCatch`
captures, indicating the effect is not identifiable. The honest response in applied work is to report the
