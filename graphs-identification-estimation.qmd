...
The graphical machinery in this chapter is Pearl's [-@pearl2009]. A directed acyclic graph, or DAG, has directed arrows such as `X -> A`. In causal work, an arrow means a direct causal relation is allowed by the model. Absence of an arrow is also a substantive assumption.

An acyclic directed mixed graph, or ADMG, adds bidirected arrows such as `A <-> Y`. ADMGs are the graphs the general identification theory of Shpitser and Pearl [-@shpitser-pearl-2006] operates on, and `causaleffect` [@tikka-hyttinen-karvanen-2019] implements that theory. A bidirected edge represents an unobserved common cause. This is useful because many econometric applications have variables we cannot measure: ability, preferences, latent health, firm quality, and so on.
...
The criterion is Pearl's [-@pearl1995], and the adjustment sets below are found with `dagitty` [@textor-dagitty-2016]. Start with a simple observational study. `A` is treatment, `Y` is the outcome, and `X` is an observed confounder:
...
The augmented inverse-probability-weighted estimator comes from Robins, Rotnitzky and Zhao [-@robins-rotnitzky-zhao-1994]; Bang and Robins [-@bang-robins-2005] give the form used here. We simulate data that match this graph, so we can check whether an estimator
...
The front-door criterion is also due to Pearl [-@pearl1995]. Now suppose treatment and outcome share an unobserved common cause. A simple adjustment argument no longer works. The front-door ADMG has a mediator `M` that transmits the effect of `A` to `Y`, while `A` and `Y` are hidden-confounded:
