...
I reuse the DGP from the estimands chapter. The true CATE is
$\tau(x)=1+2x_1$.
...
CLAN is a simple way to report heterogeneity. Sort observations by predicted
CATE, split them into bins, and estimate the ATE in each bin. If the bin
ATEs differ, the heterogeneity is not just a graph.
...
A naive causal forest ignores the firm effects:
...
For a fixed-effect style adjustment, demean by firm before fitting:
...
- BLP and CLAN are useful because they summarize heterogeneity in a way that
  looks like familiar regression output.
