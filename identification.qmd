...
To identify this, we'll need unconfoundedness: $$ E[Y(0) | W, X] = E[Y(0)| X] $$ and overlap: $$ \pi(x)= P(W=1 | X=x) < 1$$ (for the ATT only $\pi(x)<1$ is needed; the ATE requires $0<\pi(x)<1$).
...
$$ E[Y(0)| W=1] = E\big[ E[Y | W=0, X] \mid W=1 \big], $$
