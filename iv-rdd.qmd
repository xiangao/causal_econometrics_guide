...
The linear-in-$X$ 2SLS does not recover the true LATE. The polynomial 2SLS is closer, and DDML PLIV is closer still. The Wald estimator is far off because $Z$ is not unconditionally exogenous, and Wald has no way to use $X$.
...
4. **For a binary instrument, also estimate the unconditional LATE.** With instrument propensity score weighting [@sloczynski-2024],
   ...
   where $\hat p(X) = P(Z = 1 \mid X)$ is estimated nonparametrically. Stata has `ipsw`. In R it is straightforward to build once $\hat p(X)$ is in hand.
