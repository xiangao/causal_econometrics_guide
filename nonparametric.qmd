...
-   Ideally we remove that first-order sensitivity using an orthogonal influence function. Orthogonality is the first step, not the whole result: it kills the term that is linear in the nuisance error.
-   Influence-function-based (one-step/AIPW/TMLE) estimators are orthogonal to first-order nuisance error by construction. They are $\sqrt n$ consistent and *efficient* only under further conditions: the influence function used must be the efficient one, the product of the two nuisance errors must be $o(n^{-1/2})$, the propensity score must be bounded away from zero and one, and the empirical-process term must be controlled by cross-fitting or a complexity restriction. Each of these is stated where it is used below.
...
\; + \; \underbrace{(P_n - P)[\hat\mu - \mu]}_{\text{empirical process}} .
...
The third term is the empirical-process term, and it is negligible only
conditionally. It is $o_P(n^{-1/2})$ under cross-fitting, or under a Donsker
condition restricting the complexity of the class $\hat\mu$ is drawn from. Fit a
flexible learner on the same data used to average, and it need not be small at
all.
