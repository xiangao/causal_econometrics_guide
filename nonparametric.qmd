...
Since we know the influence function, we can calculate its variance. This variance is the same as variance of $\hat \psi_1$ since they differ by a constant $\psi_1$. We can use any estimators to estimate the nuisance parameters, then get $\hat \psi_1$. Then the sample variance is the variance of the influence function; and the sample mean is $\hat\psi_1$ (differencing the two arms gives the ATE).
