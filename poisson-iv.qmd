...
1. **First stage** — project `time` on `phone`, `frfam`, and fixed effects via `feols`. The residual $\hat{v}_2 = \text{time} - \hat{\text{time}}$ satisfies $E(\mathbf{z}'\hat{v}_2)=0$ by construction. A linear first stage is valid here and remains valid even when $y_2$ is binary (Part 2).
...
| **C** | Any model | Fitted values $\hat{p}_2$ | Exogeneity of $\mathbf{z}$ only | Structural mean $\exp(\cdots)$ correct; multiplicative error mean-independent of $\mathbf{z}$ |
...
cat("Approach C is also close: the omitted ad and female effects are\n")
cat("independent multiplicative heterogeneity, absorbed by the intercept\n")
cat("under the multiplicative moments — omitting FE causes no slope bias\n")
cat("here. With many groups, Approach A remains the practical choice.\n")
```
