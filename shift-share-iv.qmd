...
n_region   <- 500
n_industry <- 20
beta_true  <- 0.5       # true effect of regional shock on outcome

# Shares: each region's industry mix. Use Dirichlet(0.3) so that regions
# concentrate in a few industries (more realistic than uniform mixing).
shares <- t(replicate(n_region, {
  raw <- rgamma(n_industry, shape = 0.3)
  raw / sum(raw)
}))
colnames(shares) <- paste0("s", 1:n_industry)

# Industry-level shocks (the "shifts").
shocks <- rnorm(n_industry, mean = 0, sd = 1.0)

# Construct the shift-share instrument B_l = sum_k s_lk * g_k
B <- as.numeric(shares %*% shocks)

# Endogenous local shock: depends on B plus a confounder
u <- rnorm(n_region)
X <- B + 0.3 * u + rnorm(n_region, sd = 0.3)

# Outcome: true effect beta_true * X, plus confounder, plus noise
Y <- beta_true * X + u + rnorm(n_region, sd = 0.5)

df <- tibble(region = 1:n_region, X = X, Y = Y, B = B)
head(df)
```
...
X_bad <- B_bad + 0.3 * u + rnorm(n_region, sd = 0.3)
Y_bad <- beta_true * X_bad + u + 0.6 * v + rnorm(n_region, sd = 0.5)
